...
function StaffRequests() {
  ...
  const act = (id: string, status: "approved" | "rejected") => { setUserStatus(id, status); force(n => n + 1); };

  return (
    <AppShell title="Staff requests & roster">
      <section>
        <h2 className="text-lg mb-3">Pending approvals <span className="text-muted-foreground text-sm">({pending.length})</span></h2>
        {pending.length === 0 ? (
          <div className="glass rounded-2xl p-8 text-center text-sm text-muted-foreground">No pending requests.</div>
        ) : (
              ...
              <div key={s.id} className="glass-strong rounded-2xl p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-muted-foreground truncate">{ROLE_LABEL[s.role]} · {s.department}</div>
                  </div>
                  <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-warning/15 text-warning">pending</span>
                </div>
                ...
                <div className="mt-4 flex gap-2">
                  <button onClick={() => act(s.id, "approved")} className="btn-primary flex-1 rounded-xl py-2 text-sm">Approve</button>
                  <button onClick={() => act(s.id, "rejected")} className="flex-1 rounded-xl py-2 text-sm border border-destructive/40 text-destructive hover:bg-destructive/10">Reject</button>
                </div>
              </div>
            ))}
          </div>
              ...
              {others.map(s => (
                <tr key={s.id} className="border-t border-white/5">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full ${s.status === "approved" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}>{s.status}</span>
                  </td>
                </tr>
              ))}
              {others.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-6 text-center text-muted-foreground text-sm">No staff yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function KV({ k, v }: { k: string; v: string }) {
  return (<><dt className="text-muted-foreground">{k}</dt><dd className="truncate">{v}</dd></>);
}
