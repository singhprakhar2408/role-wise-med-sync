  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      ...
      <input
        ...
        onBlur={() => { if (value.trim()) learn(hosp, field, value); }}
        ...
        <div className="absolute z-30 mt-1 left-0 right-0 glass-strong rounded-xl p-1 max-h-56 overflow-auto">
          {items.map(it => (
            <button
              type="button"
              key={it}
              onMouseDown={(e) => { e.preventDefault(); onChange(it); learn(hosp, field, it); setOpen(false); }}
              className="w-full text-left text-sm px-3 py-2 rounded-lg hover:bg-white/10 flex items-center justify-between"
            >
              <span>{it}</span>
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">{field.replace("_", " ")}</span>
            </button>
          ))}
        </div>
      )}
